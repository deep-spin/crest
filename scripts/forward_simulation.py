...
def collate_fn(data):
    if len(data[0]) == 4:
        ...
        x0, x1, y, z, l = list(zip(*data))
        x0 = torch.nn.utils.rnn.pad_sequence(x0, batch_first=True, padding_value=0)
    ...
    def __init__(self, fname, tokenizer, split_size=None):
        ...
        try:
            ...
            self.df = pd.read_csv(
                fname,
                sep='\t',
                usecols=['tokens', 'labels', 'predictions', 'z']
            )
            self.df = self.df.rename(columns={"tokens": "orig_texts",
                                              "labels": "orig_labels",
...
class BowNLIRationaleDataset(BowRationaleDataset):
    def __getitem__(self, idx):
        row = self.df.iloc[idx]

        parts = row['orig_texts'].strip().split('</s>')
        prem_tokens = parts[0].strip().split()
        hyp_tokens = parts[1].strip().split()

        prem_input_ids = self.tokenizer.convert_tokens_to_ids(prem_tokens)
        hyp_input_ids = self.tokenizer.convert_tokens_to_ids(hyp_tokens)
        clf_label = self.label_map[row['orig_predictions']]

        rationale = eval(row['orig_z'])
        rationale = rationale[len(prem_tokens) + 1:len(prem_tokens) + 1 + len(hyp_tokens)]
        message = [
            idx for idx, z in zip(hyp_input_ids, rationale)
            if z > 0 and idx not in [self.tokenizer.pad_token_id, self.tokenizer.eos_token_id]
        ]

        x_prem = torch.tensor(prem_input_ids)
        x_hyp = self.make_bow_vector(message)
        y = torch.tensor(clf_label)
        z = torch.tensor(rationale)
        l = torch.tensor(len(x_prem))
        return x_prem, x_hyp, y, z, l
...
class BowNLIStudent(Student):
    def __init__(self, emb_layer, message_size, nb_classes, lr=0.001, weight_decay=0.0, hidden_size=50,
        ...
        self.linear_hyp = nn.Linear(message_size, nb_classes)
        self.linear_prem = nn.Linear(n * hidden_size, nb_classes)
        self.linear_merge = nn.Linear(nb_classes, nb_classes)
    ...
    def forward(self, x, z=None, l=None):
        ...
        if self.lstm.bidirectional:
            h_prem = torch.cat([hx[-2], hx[-1]], dim=-1)
        else:
            h_prem = hx[-1]
        h_prem = self.linear_prem(h_prem)

        # deal with hyp
        h_hyp = self.linear_hyp(x_hyp)

        # merge
        return self.linear_merge(torch.tanh(h_prem + h_hyp))
...
if __name__ == '__main__':
    ...
    elif args.student_type == "bow_nli":
        train_dataset = BowNLIRationaleDataset(args.train_data, t5_tokenizer, split_size=0.9)
        val_dataset = BowNLIRationaleDataset(args.train_data, t5_tokenizer, split_size=0.1)
        test_dataset = BowNLIRationaleDataset(args.test_data, t5_tokenizer, split_size=None)
    else:
    ...
    train_dataloader = data.DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=collate_fn, shuffle=True)
    val_dataloader = data.DataLoader(val_dataset, batch_size=args.batch_size, collate_fn=collate_fn, shuffle=False)
    test_dataloader = data.DataLoader(val_dataset, batch_size=args.batch_size, collate_fn=collate_fn, shuffle=False)
